...
class Case:
  ...
  def __load_cases(self):
    ...
    for i in range(len(cases)):
      self._cases.append({key: value for key, value in cases[i].__dict__.items() if not key.startswith("_") and not key.startswith("__") and not key.startswith("db")})
      self._names.append({key: value for key, value in cases[i].__dict__.items() if key == 'name'})
