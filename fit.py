...
def get_shared_temp_dir():
    """Restituisce una cartella temporanea accessibile da tutti gli utenti."""
    if get_platform() == "win":
...
def notify_fit_bootstrap(fit_bootstrap_pid):
    """Crea il file fit_ready_<pid> nella cartella condivisa e imposta i permessi per renderlo cancellabile da tutti."""
    temp_dir = get_shared_temp_dir()
    os.makedirs(temp_dir, exist_ok=True)  # Crea la cartella se non esiste
    ready_file = os.path.join(temp_dir, f"fit_ready_{fit_bootstrap_pid}")

    try:
        with open(ready_file, "w") as f:
            f.write("ready")
        print(f"File {ready_file} creato con successo.")

        if get_platform() == "win":
            # Modifica i permessi su Windows per rendere il file cancellabile da tutti
            subprocess.run(["icacls", ready_file, "/grant", "Everyone:F"], check=True)
            print(f"Permessi aggiornati per {ready_file} su Windows.")
        else:
            # Modifica i permessi su macOS/Linux per rendere il file cancellabile da tutti
            os.chmod(
                ready_file, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO
            )  # chmod 777
            print(f"Permessi aggiornati per {ready_file} su macOS/Linux.")

    except Exception as e:
        print(f"Errore nella creazione del file {ready_file}: {e}")
