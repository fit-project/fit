...
class PostAcquisition(QtCore.QObject):
    ...
    def generate_timestamp_report(self, folder, case_info, type):
        ...
        if options["enabled"]:
            ...
            self.thread_timestamp.started.connect(
                lambda: timestamp.apply_timestamp(folder, "acquisition_report.pdf")
            )

            timestamp.finished.connect(timestamp.deleteLater)
            timestamp.finished.connect(self.thread_timestamp.quit)

            self.thread_timestamp.finished.connect(
                lambda: self.__thread_timestamp_is_finished(folder, case_info, type)
            )
